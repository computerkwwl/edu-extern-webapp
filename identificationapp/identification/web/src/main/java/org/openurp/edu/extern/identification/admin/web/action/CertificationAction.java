...
public class CertificationAction extends CertificationSupportAction<Integer, Certification> {
  ...
  private String apply3(Long stdId) {
    ...
    try {
      ExternExamSubjectSetting setting = entityDao.get(ExternExamSubjectSetting.class, "examSubject", examSubject).get(0);
      put("setting", setting);
  ...
  private String apply4(Long stdId, Integer settingId) {
    ...
    for (ExternExamSubjectField field : setting.getRequestFields()) {
      if (StringUtils.equals(IdentificationConst.ID_NUMBER, field.getInnerField())) {
        ...
        if (StringUtils.isNotBlank(var)) {
          ...
          builder.where("certificate." + var + ".id = :" + field.getInnerField()
              + "Id or certificate." + var + " is null", id);
    ...
    put("certifications", entityDao.get(Certification.class, new String[] { "std", "certificate" }, new Object[] { std, certificate }));
    put("std", std);
    put("setting", setting);
  ...
  public String apply4ResultAjax() {
    ...
    if (StringUtils.isBlank(idNumber) || StringUtils.isBlank(certCode)
      ...
      for (String var : IdentificationConst.certFields) {
        for (ExternExamSubjectField field : setting.getRequestFields()) {
          if (StringUtils.equals(var, field.getInnerField())) {
            certItems.add(new Object[] { field, entityDao.get(CERT_ELEMENT_OBJECT_MAP.get(field.getInnerField()), getIntId(field.getInnerField())) });
            break;
          }
        }
      }
        ...
        for (ExternExamSubjectField field : setting.getRequestFields()) {
          if (StringUtils.equals(innerField, field.getInnerField())) {
            ...
            builder.where("certificate." + var + ".code = :" + var + "Code or certificate." + var
                + " is null", code);
            certItems.add(new Object[] { field, entityDao.get(CERT_ELEMENT_OBJECT_MAP.get(field.getInnerField()), "code", code).get(0) });
            break;
          }
        }
      }
